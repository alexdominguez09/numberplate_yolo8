"""
HEADLESS SPANISH LICENSE PLATE RECOGNITION SYSTEM - v2.02
...
class HeadlessSpanishLPR_v2_02:
    """
    Headless Spanish License Plate Recognition system v2.02
    With improved OCR accuracy and per-car deduplication.
    """
    
    def __init__(self, video_path: str, output_csv: str = "results_spanish_headless_v2.02.csv",
                 output_text_file: str = "unique_plates_v2.02.txt",
    ...
    def preprocess_plate_improved(self, plate_crop: np.ndarray) -> list:
        ...
        denoised = cv2.fastNlMeansDenoise(gray, None, h=3, templateWindowSize=7, searchWindowSize=21)
        _, thresh_denoise = cv2.threshold(denoised, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        methods.append((thresh_denoise, 'denoise_otsu'))
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Headless Spanish License Plate Recognition System v2.02 - IMPROVED'
    )
    ...
    parser.add_argument('--output', '-o',
                        type=str,
                        default='results_spanish_headless_v2.02.csv',
                        help='Output CSV file path')
    parser.add_argument('--plates-file', '-pf',
                        type=str,
                        default='unique_plates_v2.02.txt',
...
def main():
    """
    Main function for headless Spanish LPR system v2.02.
    """
    # Parse arguments
    args = parse_args()
    ...
    print("\n" + "=" * 70)
    print(f"🚀 HEADLESS SPANISH LPR - v2.02 (IMPROVED)")
    print("=" * 70)
    ...
    lpr_system = HeadlessSpanishLPR_v2_02(
    ...
    # Summary
    print(f"\n📈 SUMMARY:")
    print(f"   • Version: v{report.get('version', '2.02')}")
